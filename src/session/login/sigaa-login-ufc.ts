...
/**
 * Responsible for logging in UFC.
 * @category Internal
 */
export class SigaaLoginUFC implements Login {
  ...
  async getLoginForm(): Promise<SigaaForm> {
    if (this.form) {
      ...
      const page = await this.http.get('/sigaa/logar.do?dispatch=logOff');
  ...
  protected async parseDesktopLoginResult(page: Page): Promise<Page> {
    ...
    if (accountPage.bodyDecoded.includes('action="/sigaa/logar.do?dispatch=logOn"')) {
